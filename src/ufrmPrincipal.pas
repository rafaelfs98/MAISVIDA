...
begin
  if dm.sCadastrado = 'true' then
     ...
     begin
      ShowMessage('Atenção faça seu cadastro para obter seus descontos !!!');
...
begin
  if not dm.VerificaInternet then
     ShowMessage('Você está sem internet');
...
begin
   diag.Show(TIconDialog.Question, 'Atenção',
            'Deseja fechar este aplicativo?', 'Sim', ClickFechar, 'Não', ClickCancel);
end;
        ...
        if TListItemImage(ItemObject).Name = 'Image4' then
          begin
             ...
             diag.Show(TIconDialog.Question, 'Pergunta',
              'Confirma a exclusão do Item ?', 'Sim', ClickOK, 'Não', ClickCancel);
          end;
       end;
    end;
end;
...
begin
  ...
  try
      ...
      if(retorno.FCodigo = 200)then
        ...
        begin
          diag.Show(TIconDialog.Error,'Desculpe','Não consegui executar essa tarefa');
        end;
...
begin
   with lvItens.Items.Add do
    begin
        Detail := codigo;
        TListItemText(Objects.FindDrawable('Text1')).Text := descricao;
        TListItemText(Objects.FindDrawable('Text3')).Text := 'Preço R$ '+FormatFloat('0.00',preco.ToDouble());
       // TListItemText(Objects.FindDrawable('Text6')).Text := 'preço';
      //  TListItemText(Objects.FindDrawable('Text7')).Text := 'preço no Club';
        TListItemText(Objects.FindDrawable('Text4')).Text := 'Preço no clube R$ '+FormatFloat('0.00',promocao.ToDouble());
